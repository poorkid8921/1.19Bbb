...
public class ReportInventory extends InventoryInstanceReport {
    ...
    public Inventory getInventory() {
        ...
        inv.setItem(10, Report.createitemstack(Material.END_CRYSTAL,
                "Cheating",
                List.of(translateo("&aUse of a hacked client"),
                        translateo("&aUse of an illegal client modification"),
                        translateo("&aUse of walksy optimizer below 200ms")),
                arg));

        inv.setItem(11, Report.createitemstack(Material.PAPER,
                "Doxxing",
                List.of(translateo("&aSaying private info of a player in the chat")),
                arg));

        inv.setItem(12, Report.createitemstack(Utils.getHead(arg),
                "Ban Evading",
                List.of(translateo("&aUsing an alt to play after being banned")),
                arg));

        inv.setItem(13, Report.createitemstack(Material.RED_BED,
                "Spamming",
                List.of(translateo("&aSaying more than 5 messages on the same topic")),
                arg));
