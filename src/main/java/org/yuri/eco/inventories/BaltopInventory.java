...
public class BaltopInventory extends InventoryInstance {
    ...
    public Inventory getInventory() {
        ...
        Inventory inv = Bukkit.createInventory(this, 54, Utils.translateo(page + " / " + maxPage + " - ᴛᴏᴘ ʙᴀʟᴀɴᴄᴇs"));
        int min = (page - 1) * 21;
        int max = page * 21;
        ...
        for (int j = min; j < max && j < 4613; j++) {
            ...
            String name = PlaceholderAPI.setPlaceholders(null, "%ajlb_lb_vault_eco_balance_commas_" + index + "_alltime_name%");
            String money = PlaceholderAPI.setPlaceholders(null, "%ajlb_lb_vault_eco_balance_commas_" + index + "_alltime_value%");
        ...
        if (page < maxPage) inv.setItem(26, new ItemStack(Material.GREEN_CONCRETE));
        if (page > 1) inv.setItem(18, new ItemStack(Material.RED_CONCRETE));
        ...
        Bukkit.getScheduler().runTaskAsynchronously(Initializer.p, () -> {
            ...
            skull.setDisplayName(Utils.translate("#46FF64#" + Integer.parseInt(PlaceholderAPI.setPlaceholders(null, "%ajlb_position_vault_eco_balance_commas_alltime%")) + " &f" + player.getDisplayName()));
            skull.setLore(List.of(Utils.translateo("&fᴍᴏɴᴇʏ: &a$" + PlaceholderAPI.setPlaceholders(null, "%ajlb_lb_vault_eco_balance_commas_alltime_value%"))));
    ...
    public int getMaxPage() {
        return Math.max(1, (int) Math.ceil(4613.0D / 21d));
    }
