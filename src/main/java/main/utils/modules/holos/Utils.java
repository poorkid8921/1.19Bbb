...
public class Utils {
    public static final LoopableHologramHolder[] holos = new LoopableHologramHolder[2];
    public static final LoopableHologramHolder[] tickableHolos = new LoopableHologramHolder[6];
    public static final ClientboundAddEntityPacket[] itemHolos = new ClientboundAddEntityPacket[1];

    public static void init() {
        LeaderboardSync.sort();
        /*Bukkit.getLogger().warning("top_deaths: " + top_deaths.get(0).second());
        Bukkit.getLogger().warning("top_kills: " + top_kills.get(0).second());
        Bukkit.getLogger().warning("top_playtime: " + getTimeLB(top_playtime.get(0).second()));
        Bukkit.getLogger().warning("local_stored_deaths " + leaderboardData.get("Catto69420").getDeaths());
*/
        //int i = -1;
        //int itemI = -1;
    ...
    public static void showForPlayer(ServerGamePacketListenerImpl connection) {
        for (final LoopableHologramHolder k : holos) {
    ...
    public static void showForPlayerTickable(ServerGamePacketListenerImpl connection) {
        for (final LoopableHologramHolder k : tickableHolos) {
            connection.send(k.ADD_ENTITY);
            connection.send(k.METADATA);
        }
        for (final ClientboundAddEntityPacket k : itemHolos) {
            connection.send(k);
        }
    }
    ...
    static class HologramHolder {
        ...
        private final Item hoverItem;

        public HologramHolder(final String[] lines, final double x, final double y, final double z, final Item hoverItem) {
            this.lines = lines;
            this.x = x;
            this.y = y;
            this.z = z;
