    ...
    public void onPacketPlayReceive(PacketPlayReceiveEvent event) {
        ...
        switch (new WrapperPlayClientInteractEntity(event).getEntityId()) {
            ...
            case 1 -> {
                ...
                playersUsingNPC.add(name);
                p.teleportAsync(Initializer.flat, PlayerTeleportEvent.TeleportCause.COMMAND).thenAccept(r -> {
                    inFlat.add(p);
                    playersUsingNPC.remove(name);
                });
            }
            ...
            case 3 -> {
                ...
                if (playersUsingNPC.contains(name))
                    return;
                playersUsingNPC.add(name);
                p.teleportAsync(nethpot, PlayerTeleportEvent.TeleportCause.COMMAND).thenAccept(r -> playersUsingNPC.remove(name));
            }
        }
    }
}