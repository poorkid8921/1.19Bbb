package ac.checks.impl.velocity;
...
import ac.utils.data.VectorData;
import ac.utils.data.VelocityData;
...
public class KnockbackHandler extends Check implements PostPredictionCheck {
    ...
    public void onPacketSend(final PacketSendEvent event) {
        if (event.getPacketType() == PacketType.Play.Server.ENTITY_VELOCITY) {
            ...
            GrimPlayer player = Practice.PDM.getPlayer(event.getUser());
    ...
    public void onPredictionComplete(final PredictionComplete predictionComplete) {
        ...
        if (player.likelyKB != null) {
            if (player.likelyKB.offset > 0.001) {
                threshold = Math.min(threshold + player.likelyKB.offset, 4);
                ...
                } else if (flag() && player.likelyKB.offset >= 0.1 || threshold >= 1)
                        player.getSetbackTeleportUtil().executeViolationSetback();
                else {
                    reward();
                }
    ...
    public boolean shouldIgnoreForPrediction(VectorData data) {
        if (data.isKnockback() && data.isFirstBreadKb()) {
            return player.firstBreadKB.offset > 0.001;
        }
        return false;
    }

    public boolean wouldFlag() {
        return (player.likelyKB != null && player.likelyKB.offset > 0.001) || (player.firstBreadKB != null && player.firstBreadKB.offset > 0.001);
    }
