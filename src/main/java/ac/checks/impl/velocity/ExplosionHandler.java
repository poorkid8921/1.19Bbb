...
public class ExplosionHandler extends Check implements PostPredictionCheck {
    ...
    public boolean shouldIgnoreForPrediction(VectorData data) {
        if (data.isExplosion() && data.isFirstBreadExplosion()) {
            return player.firstBreadExplosion.offset > 0.00001;
        }
        return false;
    }

    public boolean wouldFlag() {
        return (player.likelyExplosions != null && player.likelyExplosions.offset > 0.00001) || (player.firstBreadExplosion != null && player.firstBreadExplosion.offset > 0.00001);
    }
    ...
    public void onPredictionComplete(final PredictionComplete predictionComplete) {
        ...
        if (player.likelyExplosions != null) {
            if (player.likelyExplosions.offset > 0.00001) {
                if (flag() && getViolations() > 10)
