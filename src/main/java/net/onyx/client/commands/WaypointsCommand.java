...
public class WaypointsCommand extends CommandNode {
    ...
    public static class Show extends SubCommand {
        ...
        private void displayWaypoint(String name) {
            WaypointSystem.Waypoint waypoint = system.getWaypoint(name);
    ...
    public static class Add extends SubCommand {
        ...
        public Boolean trigger(String[] args) {
            ...
            Vec3d pos;
            if (current) {
                pos = OnyxClient.me().getPos();
            } else {
                pos = args.length == 3 
                    ? this.vec3dFromString(args[1], "0", args[2]).add(0, OnyxClient.me().getY(), 0)
            ...
            boolean added = system.addWaypoint(name, new WaypointSystem.Waypoint(pos));
            if (!added) {
